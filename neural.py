...
class SSingleMatchNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        att_norm = self.masked_softmax(att_weights, seq_len)

        att_vec = att_norm.bmm(proj_q)
        output = nn.ReLU()(self.trans_linear(att_vec))
        return output

    def masked_softmax(self, vector, seq_lens):
        ...
        # mask = None
        if mask is None:
            result = torch.softmax(vector, dim=-1)
        else:
            result = torch.softmax(vector * mask, dim=-1)
            result = result * mask
...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, head_count, model_dim, dropout=0.1, use_final_linear=True):
        ...
        if (self.use_final_linear): self.final_linear = nn.Linear(model_dim, model_dim)
